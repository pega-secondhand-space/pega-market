    ...
    function renderItems() {
      ...
      const filtered = allItems.filter(item => {
        ...
        if (itemExp) {
          ...
          if (expDate < new Date()) return false; // Filter out expired items
        }
      ...
      grid.innerHTML = pageItems.map(item => {
        ...
        return `
          <div onclick="openDetailModal('${item.id}')" class="bg-gray-900/95 border ${isPinned ? 'border-amber-500/80 shadow-amber-500/10 shadow-lg' : 'border-gray-800'} rounded-2xl overflow-hidden flex flex-col transition hover:border-indigo-500/60 cursor-pointer group">
            ${isPinned ? '<div class="bg-amber-500 text-gray-950 text-xs sm:text-sm font-black px-3 py-1 text-center">📌 官方置頂商品</div>' : ''}
            
            <div class="relative aspect-video bg-gray-950 overflow-hidden flex items-center justify-center">
              <img id="card-img-${item.id}" src="${(item.image_url || '').split('|||')[0] || 'https://images.unsplash.com/photo-1526170375885-4d8ecf77b99f?w=600&auto=format&fit=crop'}" class="max-w-full max-h-full object-contain group-hover:scale-105 transition-transform duration-300">
            ...
            <div class="p-4 flex-1 flex flex-col justify-between space-y-3">
              <div>
                <h3 class="font-black text-base sm:text-lg text-gray-100 line-clamp-1 mb-1.5 group-hover:text-indigo-300 transition leading-snug">${safeTitle}</h3>
                <p class="text-base sm:text-lg text-indigo-400 font-black mb-2">${item.type === 'free' ? 'NT$ 0 (免費親自取件)' : `NT$ ${(item.price || 0).toLocaleString()}`}</p>
                <p class="text-xs sm:text-base text-gray-300 line-clamp-2 leading-relaxed font-medium">${safeDesc || '無補充描述'}</p>
              </div>
              ...
              <div class="space-y-2.5 pt-3 border-t border-gray-800/80">
                ...
                <div class="flex items-center justify-between text-xs sm:text-sm text-gray-400 font-medium">
                  <span>${item.nickname || '匿名同仁'}</span>
                  <span class="text-amber-400/90 font-bold">${getExpirationCountdown(getItemExpiration(item))}</span>
                </div>

                ${(item.device_id === deviceId || (JSON.parse(localStorage.getItem('my_created_item_ids') || '[]')).includes(item.id)) ? `
                  <button onclick="event.stopPropagation(); deleteMyItem('${item.id}')" class="w-full bg-red-950/80 hover:bg-red-900 text-red-300 border border-red-700/60 text-xs sm:text-sm py-2 rounded-xl font-bold transition flex items-center justify-center gap-1.5 shadow-sm">
                    <i class="fa-solid fa-trash-can"></i> 下架我的貼文 (已售出)
                  </button>
                ` : ''}

                ${isAdmin ? `
                  <div class="flex gap-1.5 pt-1" onclick="event.stopPropagation()">
                    <button onclick="togglePinItem('${item.id}')" class="flex-1 ${isPinned ? 'bg-amber-600' : 'bg-gray-800 text-amber-300'} text-xs sm:text-sm py-1.5 rounded-xl font-bold transition">${isPinned ? '取消置頂' : '📌 置頂'}</button>
                    <button onclick="deleteItemAdmin('${item.id}')" class="px-3 bg-red-900/60 text-red-300 text-xs sm:text-sm py-1.5 rounded-xl font-bold transition">🗑️ 刪除</button>
                  </div>
                ` : ''}
              </div>
            </div>
    ...
    function getCleanDescription(desc) {
      if (!desc) return '';
      return desc.replace(/\[EXP:.*?\]/g, '').trim();
    }
    ...
    function checkAdminPassword() {
      ...
      if (validPasswords.includes(pwd)) {
        isAdmin = true;
        const pwdView = document.getElementById('admin-pwd-view');
        ...
        // 即時預填當前畫面公告內文
        const liveAnnText = document.getElementById('announcement-text')?.innerText || '';
